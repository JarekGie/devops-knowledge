...
Sprawdź pokrycie tagów i gotowość governance. Status: `GO` = spełnione, `PARTIAL` = częściowe braki, `NO-GO` = sprawdzone i niespełnione, `GAP` = brak kontroli względem standardu (governance gap, brak aktywnego incydentu), `niezweryfikowane` = nie sprawdzono.

**Źródło historyczne:** [[rshop-tagging-baseline-2026-04-24]]
**Bieżący scan:** sample-based (~15 zasobów sprawdzonych live: resourcegroupstaggingapi + S3 get-bucket-tagging)

| Obszar | Status | Uwagi |
|--------|--------|-------|
| FinOps — cost allocation tags (Project/Environment/CostCenter) | PARTIAL | Prod: ✅ pełne. Dev CFN stacks: brak CostCenter. Dev S3: ✅ |
...
| ECS/Fargate — tag propagation do tasków (`propagate_tags`) | PARTIAL | propagateTags=SERVICE potwierdzone na 1/10 serwisów (rshop-prod-api-svc). Pozostałe 9: niezweryfikowane |
...
| AWS WAF — obecność i przypisanie właściciela | GAP | 0 WAF ACLs (sprawdzone live: wafv2 REGIONAL eu-central-1 + CLOUDFRONT us-east-1). Brak WAF = governance gap, nie aktywna awaria |
