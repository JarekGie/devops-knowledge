...
| Prompt injection z dokumentu wejściowego | Złośliwa lub przypadkowa instrukcja jest ukryta w treści dokumentacji, komentarzu, tabeli albo bloku kodu. | Model może potraktować dokument jako instrukcję zamiast materiał do analizy. | Średnie | Wysoki | Oznaczać dokument jako dane, nie instrukcje; stały system prompt; skan prompt injection przed eksportem. |
| Data exfiltration through prompt content | Treść dokumentu próbuje wymusić wypisanie mapowań, sekretów lub pełnej treści wejściowej. | Ujawnienie danych albo obejście kontraktu LLM. | Średnie | Krytyczny | Nigdy nie wysyłać mapowań do LLM; filtrować payloady; walidować odpowiedź przed rehydratacją. |
| Mapping store becomes crown jewels | Store mapowań zawiera pełny most między tokenami i danymi oryginalnymi. | Kompromitacja store oznacza deanonimizację wszystkich wyników. | Średnie | Krytyczny | Szyfrowanie, split access, krótka retencja, audyt rehydratacji. |
| Semantyczne tokeny zdradzają za dużo | Token typu `AWS_ACCOUNT_ID_003` ujawnia technologię lub klasę zasobu. | Częściowy wyciek kontekstu technicznego. | Średnie | Średni/Wysoki | Konfigurowalna taksonomia tokenów; tryb tokenów ślepych dla wyższych klas danych. |
| Błędna rehydratacja zmienia sens wyniku | Token zostanie podstawiony złą wartością albo w złym miejscu. | Operator otrzymuje błędny dokument lub rekomendację. | Niskie/Średnie | Wysoki | Deterministyczne mapowanie, walidacja tokenów, audit trail, testy rehydratacji. |
