...
class UdemyBrowser:
    def __init__(
        ...
        self._cdp_mode = cdp_url is not None
    ...
    async def __aenter__(self) -> "UdemyBrowser":
        ...
        if self._cdp_mode:
            # Połącz z istniejącą instancją Chrome uruchomioną z --remote-debugging-port
            # Omija Cloudflare — przeglądarka ma już cf_clearance i sesję Udemy
            logger.info("Tryb CDP: łączenie z %s", self.cdp_url)
            self._browser = await self._playwright.chromium.connect_over_cdp(self.cdp_url)
            contexts = self._browser.contexts
            if contexts:
                self._context = contexts[0]
                logger.info("CDP: używam istniejącego kontekstu (%d stron)", len(self._context.pages))
            else:
    ...
    async def __aexit__(self, *_: Any) -> None:
        if self._cdp_mode:
            # Nie zamykaj zewnętrznej przeglądarki — tylko odłącz
            if self._browser:
    ...
    async def save_state(self) -> None:
        if self._cdp_mode:
            logger.debug("CDP: pomijam zapis sesji (zewnętrzna przeglądarka)")
            return
        assert self._context is not None
    ...
    async def is_logged_in(self, page: Page) -> bool:
        if self._cdp_mode:
            # W trybie CDP przeglądarka jest już zalogowana — szybka weryfikacja
            try:
                await page.goto("https://www.udemy.com/", wait_until="domcontentloaded", timeout=30000)
                await page.wait_for_selector('[data-purpose="user-dropdown"]', timeout=10000)
                return True
            except Exception:
                return False
