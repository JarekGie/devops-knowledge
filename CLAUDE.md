...
## Zasada autorstwa (non-negotiable)

AI jest wyłącznie narzędziem roboczym. Autorami i ownerami wszystkich artefaktów są wyłącznie ludzie.

**Zabronione we wszystkich artefaktach** (repo, vault, commity, PR-y, ADR-y, plany, changelog):
- `Author: ... + Claude` lub `Author: Claude`
- `Co-authored-by: Claude` / `Co-Authored-By: Claude Sonnet <...>`
- `Generated by Claude`, `Prepared by Claude`
- Wzmianka o AI w commit message, PR description, changelog

**Dla tego projektu — jedyny dozwolony autor:** `Jarosław Gołąb`

**Styl formułowania rekomendacji:**
- Zamiast „moja rekomendacja" / „zrobiłem" → używaj „rekomendowany wariant" / „proponowany plan" / „wynik analizy wskazuje"
- AI wspiera pracę operatora — nie jest współautorem ani głównym wykonawcą
